...
if (isset($this->variables[0])) {
	...
	if ($obj!==false) {
		...
		<p>Released: <?=date($t->t('Y-m-d'), $obj['releaseDate'])?></p>
...
<div class="span2 browsers">
	...
	<h6>(<?=date($t->t('Y-m-d'), $browser['stableUpdate'])?>)</h6 -->
	<br>
	<ul class="unstyled">
		<?php
		if (isset($history[$browser['id']][$branchId])) {
			...
			?>
				<li><a href="<?=$this->link('/history/'.$historyObj['id'])?>" title="<?=$browser['name'].' '.$historyObj['releaseVersion']?>"><?=$historyObj['releaseVersion']?></a> <span class="date"><?=date($t->t('Y-m-d'), $historyObj['releaseDate'])?></span><?=$this->edit?' <a href="'.$this->link('/edit/'.$historyObj['id']).'" class="icon-edit"></a> <a href="'.$this->link('/remove/'.$historyObj['id']).'" class="icon-remove"></a>':''?></li>
			<?php
			}
		}
		?>
